...
private:
 ...
 bool ObjectAdministrator::checkOverlap(const Zelle& zelle, int posX, int posY, int width, int height);    //Für assignObject, hilft für einen besseren Überblick des Codes
...
std::vector<std::vector<int>> readObjectsFromFile(const std::string& filename) {
